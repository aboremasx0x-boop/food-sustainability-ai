    ...
    reader.onload = async (event) => {
      ...
      const branchArray = Object.entries(branches)
        .map(([name, data]) => ({
          ...
          score: Math.max(
            0,
            Math.min(100, Math.round(100 - data.waste * 0.8 - data.loss / 60))
          ),
        }))
      ...
      const bestBranch = branchArray[0]?.name || "-";
      const worstBranchName = branchArray[branchArray.length - 1]?.name || "-";
      const highestFood = foodArray[0]?.name || "-";
      const highestCity = cityArray[0]?.name || "-";
      const strongestPath = pathArray[0]?.name || "-";
      ...
      setWorstBranch(worstBranchName);
      setTopFood(highestFood);
      setTopCity(highestCity);
      ...
      const { error } = await supabase.from("analysis_reports").insert([
        {
          file_name: file.name,
          total_waste: Math.round(total),
          financial_loss: Math.round(loss),
          expected_savings: expectedSave,
          donation_kg: Math.round(donation),
          recycling_kg: Math.round(recycling),
          forecast: nextForecast,
          risk_level: risk,
          sustainability_score: finalScore,
          top_branch: bestBranch,
          worst_branch: worstBranchName,
          top_food: highestFood,
          top_city: highestCity,
          best_path: strongestPath,
        },
      ]);

      if (error) {
        setSaveStatus("لم يتم الحفظ في قاعدة البيانات. تحقق من إعدادات Supabase.");
      } else {
        setSaveStatus("تم حفظ نتائج التحليل في قاعدة البيانات بنجاح.");
      }
...
• أفضل فرع: ${bestBranch}
• أسوأ فرع: ${worstBranchName}
• أكثر نوع طعام هدرًا: ${highestFood}
• أعلى مدينة في الهدر: ${highestCity}
• أفضل مسار استفادة: ${strongestPath}
      ...
      <div style={{ maxWidth: "1400px", margin: "0 auto" }}>
        ...
        >
          ...
          <Card
            title="Risk Level"
            value={riskLevel}
            color={riskLevel === "High" ? "#dc2626" : riskLevel === "Medium" ? "#f59e0b" : "#16a34a"}
            darkMode={darkMode}
          />
        ...
        <Section darkMode={darkMode}>
          ...
          {saveStatus && <p style={{ marginTop: "15px", color: "#22c55e" }}>{saveStatus}</p>}
        ...
        <ChartSection
          ...
          data={foodStats}
          bars={[{ key: "value", color: "#14b8a6" }]}
          darkMode={darkMode}
        />

        <ChartSection
          title="City Waste Analytics"
          data={cityStats}
          bars={[{ key: "value", color: "#06b6d4" }]}
          darkMode={darkMode}
        />
